function [errstd errps]=errmeta_dl(metamodel,parameters,datamatrix)
...
%  mysubplot(2,2,i)
  ...
  tmps=squeeze(datamatrix.valdata(indd{:},:))-repmat(tmpr,[size(tmpr)./size(tmpr) 20]);
  tmpp=squeeze(predval(indd{:},:))-repmat(tmpr,[size(tmpr)./size(tmpr) 20]);
